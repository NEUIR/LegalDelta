...
from trl.trainer.grpo_trainer import (Any, AutoModelForCausalLM, AutoModelForSequenceClassification, AutoTokenizer, Dataset, GRPOConfig, GRPOTrainer, GenerationConfig, IterableDataset, Optional, PeftConfig, PreTrainedModel, PreTrainedTokenizerBase, RepeatSampler, RewardFunc, Sampler, SyncRefModelCallback, Trainer, TrainerCallback, Union, apply_chat_template, broadcast_object_list, create_reference_model, defaultdict, gather, gather_object, generate_model_card, get_comet_experiment_url, is_conversational, is_deepspeed_zero3_enabled, is_peft_available, is_peft_model, is_wandb_available, maybe_apply_chat_template, nn, os, pad, patch, prepare_deepspeed, set_seed, textwrap, torch, transformers, unwrap_model_for_generation, version, warnings, Any, Union, apply_chat_template, broadcast_object_list, gather, gather_object, is_conversational, maybe_apply_chat_template, nn, os, pad, torch, unwrap_model_for_generation, os, torch, transformers, GRPOTrainer, Trainer, gather, os, torch)
    ...
    def extract_answer_tokens_and_probs(self, completion_ids, completion_logits, completions_text):
        ...
        for batch_idx in range(completion_ids.shape[0]):
            try:
                ...
                eoa_positions = []
                search_start = last_bracket_pos + 1
                ...
                content_start_idx = bracket_positions[-1] + 1  # ']' 
                content_end_idx = eoa_positions[0]  # '<eoa' 
        ...
        try:
            with torch.no_grad():
                ...
                if hasattr(outputs, 'logits'):
                    ...
                    for batch_idx in range(min(2, prompt_base_ids.shape[0])): 
                        print(f"\n--- 样本 {batch_idx} ---")
                    ...
                    print("baseline outputs没有logits属性!")
                    return [{'tokens': [], 'logits': [], 'text': '', 'avg_logit': 0.0} for _ in range(len(inputs))]
            ...
            with torch.no_grad():
                ...
                if hasattr(outputs, 'logits'):
                    ...
                    if completion_logits.shape[1] != completion_ids.shape[1]:
                        print(f"logits长度({completion_logits.shape[1]}) != token长度({completion_ids.shape[1]})")
